...
if st.checkbox("Show Additional Projections"):
    ...
    # TODO: remove lots of repetitive code from previous section
    st.subheader("Hospital Capacity")
    st.markdown(
        "Projected number of hospital resources."
    )

    hosp_capacity = st.number_input(
    ...
    for k, los in los_dict.items():
        ...
        resource_dict[k] = resource[k]
        ax.plot(resource.head(plot_projection_days)[k], ".-", label=k)
        ax.legend(loc=0)
    ...
    resource_df["day"] = resource_df.index
    resource_df = resource_df[["day", "hosp", "icu", "vent"]]

    resource_table = resource_df[np.mod(resource_df.index, 7) == 0].copy()
    resource_table.index = range(resource_table.shape[0])
    resource_table.loc[0, :] = 0
    resource_table = resource_table.dropna().astype(int)
    ...
    for k, word in wording_dict.items():
        try:
            cap_day = resource_df[resource_df[k] < 0]['day'].iloc[0]
            st.markdown(
    ...
    no_cap_icu_ifr = st.number_input(
        "Fatality Rate of Critical Patients Without ICU Care %", 0, 100, value=90, step=1, format="%i"
    )
